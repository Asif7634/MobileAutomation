...
//
public class GMA4_AppiumPrateek extends Appium{
	...
	float SubTotal_OutOfStockScreen;
	float SubTotal_OrderBasketScreen;
	float SubTotal_OderSummaryScreen;
	...
	public void gma4_CustLogin_Menu(Map<String, String> input)
	{
		...
		{
			System.out.println(e.getMessage());
			HtmlResult.failed("Login In to Application.", "Customer should be able to login", " Unable to login in system" + e.getMessage());
		...
		try
		{
			...
			{	
				...
				if (CheckBoxVaue.equals("1"))
				{
					...
					HtmlResult.passed("verify the object checked " + strexpectedData, "Object should be checked","Object is checked" +strexpectedData);	
				}

				else
				{
					HtmlResult.failed("verify the object checked " + strexpectedData, "Object should be checked","Object is not checked" +strexpectedData);		
				}
			} 
			else{
	...
	public void verifyAppObjectInformation(Map<String,String> input)
	{
		...
		{
			System.out.println(e.getMessage());
			HtmlResult.failed("", "", e.getMessage());
		}
	}
	...
	public void gma4_EnterCardDetails_BarclaysPage(Map<String, String> input)
	{
			...
			if (currentOS.equalsIgnoreCase("ios"))
			{
				...
				List<WebElement> cardField = driver.findElements(By.className("XCUIElementTypeOther"));
				{
					...
					for ( i=0;i<cardField.size();i++)
					{
						...
						if ((currentLocation.x==26 && currentLocation.y==379) || (currentLocation.x==23 && currentLocation.y==306))
						...
						if ((currentLocation.x==117 && currentLocation.y==379) || (currentLocation.x==100 && currentLocation.y==306))
						{
							cardField.get(i).click();
				...
				{
					String CheckBoxVaue =driver.findElement(By.className("XCUIElementTypeSwitch")).getAttribute("value");			
		...
		{
			System.out.println(e.getMessage());
			HtmlResult.failed("Entering Card Details", "Should be able to enter card details","issue occur during entering value" + e.getMessage());	
		}
	}
	...
	public void gma4_CheckOutAndScanQRCode(Map<String, String> input)
	{
		...
		{
			boolean currentFlagValue = false;
			///updated by prateek - need to go order menu hence clicking to close button
			currentFlagValue = appUIDriver.getAppObjectFound("gma4_Common_Close");
			{
			...
			if(blnMenuAccount){
				...
				if(blnMenuPayMethods){
					...
					if(!strObjProperty.equalsIgnoreCase("false"))
					{
						...
						if(	appUIDriver.clickButton("gma4_Save_Card_Edit"))
						{

							if(strOSType.equalsIgnoreCase("ios")){
								String strObjectName1 = "gma4_SaveCard_MinusDel";
								strObjectName1 =strObjectName1.trim();                        
								strMobileObjectName  = appUIDriver.getAppObjectwithLocator(strObjectName1);
								...
								if(!strMobileObjectName.equalsIgnoreCase("false")){
									...
									for(int i = 0;i<delbuttoncount;i++){
										...
										Boolean blnClickedandFound = false;
										blnClickedandFound =	appUIDriver.clickButton("gma4_SaveCard_MinusDel");
										Thread.sleep(3000);
										if(blnClickedandFound)
										{
						...
						{
							HtmlResult.failed("Not able to click Menu_Payment_methods", "Not able to click on Edit Card ", "Not able to click Menu_Payment_methods");	
						}
	...
	public void gma_VerifyProductInOrderBasketScreen(Map<String, String> input)
	{
		try
		{
			...
			if(currentFlagValue)
			{
				...
				SubTotal_OrderBasketScreen = Float.parseFloat(((strSubTotal_OrderBasketScreen.split("£"))[1].split("\\*"))[0]);
				HtmlResult.passed("Capturing 'Sub-Total' from 'Order Basket' screen.", "User should be able to capture 'Sub-Total' from 'Order Basket' screen.", "'Sub-Total' of products on 'Order Basket' screen is : '"+String.format("%.2f",SubTotal_OrderBasketScreen)+"'.");
			}
			else
			{
		...
		{
			System.out.println(e.getMessage());
			HtmlResult.failed("Clicking on Close button on Order Basket Page.", "User should be able to click on Close button on Order Basket Page.", "Unable to click on Close button on Order Basket Page."+e.getMessage());
	...
	public void gma4_VerifyPrice_NewOrderTotalScreen()
	{
		...
		{	
			...
			{
				HtmlResult.failed("Veriyfing Price on 'New Order Total' screen.", "Expected and Actual pirce value should be same.", "Expected New Total Price '"+ExpectedTotalPrice+"' actual New Total Price '"+ActualTotalPrice+"' are not matching.");
			}
		}
		catch (Exception e)
		{
		...
		try
		{
			...
			if(currentFlagValue)
			{
				...
				if (currentFlagValue)
				{
					...
					{
						HtmlResult.failed("Verifying Drink Item on Order Summary Screen.", "Drink Item '"+strDrinkItem+"' with Uplift price '"+strUpliftPrice+"' should be present on Order Summary screen.", "Drink Item '"+strDrinkItem+"' with price '"+strUpliftPrice+"' is present on Order Summary Screen.");
					}
				}
			}
	...
	public void readFinalOrderDetails()
	{
		try{
			...
			if(blnFound)
			{
				...
				if (!strProductName.equals("false"))
				{
					...
					if(!strProductPrice.equals("false"))
					{
						...
						if(!(strPaymentCardName.equalsIgnoreCase("") ||strPaymentCardName.equalsIgnoreCase("false")))
						{
						...
						{
							HtmlResult.failed("Capturing Order details.","Order details should be captured.","Details are missing for card information" + strPaymentCardName );
						}
					}
				}
				else
				{
					HtmlResult.failed("Capturing Order details.","Order details should be captured.","Details are missing for card information" + strProductName );
			...
			else
			{
				...
				HtmlResult.failed("Capturing Order details.","Order details should be captured.","Unable to capture order details.");
			}
		}
